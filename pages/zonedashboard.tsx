...
const ZoneDashboardPage: React.FC = () => {
  ...
  const handleGenerateZones = useCallback(async () => {
    try {
      ...
      console.log('Zone tree generated successfully:', data);
      ...
      const processedTree = processZoneTree(data);
      setState(prev => ({
        ...prev,
        zones: [processedTree],
        selectedZone: processedTree.id
      ...
      console.error('Error generating zone tree:', e);
      const alert: ZoneEvent = {
        id: `gen-error-${Date.now()}`,
        zoneId: '',
        zoneName: '',
        type: 'critical',
        message: `Failed to generate zone tree: ${(e as Error).message}`,
        timestamp: new Date(),
        source: 'user'
      };
      ZoneAlertSystem.getInstance().trigger(alert);
              ...
              <form onSubmit={(e) => {
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      id="prototypeZoneName"
                      value={generationForm.prototypeZoneName}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        prototypeZoneName: e.target.value
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    />
                  </div>
                  <div>
                    <label htmlFor="recursionLevel" className="block text-sm font-medium text-gray-700">Recursion Level</label>
                    <input
                      type="number"
                      id="recursionLevel"
                      value={generationForm.recursionLevel}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        recursionLevel: Number(e.target.value)
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    />
                  </div>
                  <div>
                    <label htmlFor="simAgentProfile" className="block text-sm font-medium text-gray-700">Simulation Agent Profile</label>
                    <select
                      id="simAgentProfile"
                      value={generationForm.simAgentProfile}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        simAgentProfile: e.target.value as typeof generationForm.simAgentProfile
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                      {SIM_PROFILE_OPTIONS.map(option => (
                        <option key={option} value={option}>{option}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label htmlFor="autoSimFrequency" className="block text-sm font-medium text-gray-700">Auto Simulation Frequency</label>
                    <select
                      id="autoSimFrequency"
                      value={generationForm.autoSimFrequency}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        autoSimFrequency: e.target.value as typeof generationForm.autoSimFrequency
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                      {SIM_FREQUENCY_OPTIONS.map(option => (
                        <option key={option} value={option}>{option}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label htmlFor="impactDomain" className="block text-sm font-medium text-gray-700">Impact Domain</label>
                    <select
                      id="impactDomain"
                      value={generationForm.impactDomain}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        impactDomain: e.target.value as typeof generationForm.impactDomain
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                      {IMPACT_DOMAIN_OPTIONS.map(option => (
                        <option key={option} value={option}>{option}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label htmlFor="confidentiality" className="block text-sm font-medium text-gray-700">Confidentiality</label>
                    <select
                      id="confidentiality"
                      value={generationForm.confidentiality}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        confidentiality: e.target.value as typeof generationForm.confidentiality
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                  ...
                  <div>
                    <label htmlFor="sharedWithDAO" className="block text-sm font-medium text-gray-700">Shared with DAO</label>
                    <div className="mt-1">
                      <label className="inline-flex items-center">
                        <input
                          type="checkbox"
                          id="sharedWithDAO"
                          checked={generationForm.sharedWithDAO}
                          onChange={(e) => setGenerationForm(prev => ({
                            ...prev,
                  ...
                  <div>
                    <label htmlFor="epistemicIntent" className="block text-sm font-medium text-gray-700">Epistemic Intent</label>
                    <select
                      id="epistemicIntent"
                      value={generationForm.epistemicIntent}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        epistemicIntent: e.target.value as typeof generationForm.epistemicIntent
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                      {EPISTEMIC_INTENT_OPTIONS.map(option => (
                        <option key={option} value={option}>{option}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label htmlFor="ethicalSensitivity" className="block text-sm font-medium text-gray-700">Ethical Sensitivity</label>
                    <select
                      id="ethicalSensitivity"
                      value={generationForm.ethicalSensitivity}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        ethicalSensitivity: e.target.value as typeof generationForm.ethicalSensitivity
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                      {ETHICAL_SENSITIVITY_OPTIONS.map(option => (
                        <option key={option} value={option}>{option}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    ...
                    <select
                      id="createdBy"
                      value={generationForm.createdBy}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                        createdBy: e.target.value as typeof generationForm.createdBy
                      }))}
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    >
                  ...
                  <div>
                    <label htmlFor="guardianId" className="block text-sm font-medium text-gray-700">Guardian ID</label>
                    <input
                      type="text"
                      id="guardianId"
                      value={generationForm.guardianId}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                  ...
                  <div>
                    <label htmlFor="drift" className="block text-sm font-medium text-gray-700">Drift</label>
                    <input
                      type="number"
                      id="drift"
                      step="0.01"
                      value={generationForm.drift}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                  ...
                  <div>
                    <label htmlFor="entropy" className="block text-sm font-medium text-gray-700">Entropy</label>
                    <input
                      type="number"
                      id="entropy"
                      step="0.01"
                      value={generationForm.entropy}
                      onChange={(e) => setGenerationForm(prev => ({
                        ...prev,
                  ...
                  <div>
                    <label htmlFor="ethicalFlag" className="block text-sm font-medium text-gray-700">Ethical Flag</label>
                    <div className="mt-1">
                      <label className="inline-flex items-center">
                        <input
                          type="checkbox"
                          id="ethicalFlag"
                          checked={generationForm.ethicalFlag}
                          onChange={(e) => setGenerationForm(prev => ({
                            ...prev,
                ...
                <div className="flex space-x-2">
                  ...
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2-2V6a2 2 0 002-2h14a2 2 0 002 2v4a2 2 0 00-2 2m-2-4h.01M17 16h.01" />
                    </svg>
                  </button>
                  <button
